/**
 * Created by kinkoo on 14-2-13.
 */
Ext.define('Share.view.ActionTextColumn', {
    ...
    constructor: function(config) {
        ...
        me.renderer = function(v, meta) {
            ...
            for (i = 0; i < l; i++) {
                ...
                v += '<a style="font-size: 12px; line-height: 15px;" href="javascript:void(0);"' +
                    ...
                    ' ' + (Ext.isFunction(item.getClass) ? item.getClass.apply(item.scope||me.scope||me, arguments) : (me.iconCls || '')) + '"' +
                    ((item.tooltip) ? ' data-qtip="' + (item.getText ? item.getText.apply(item.scope||me.scope||me, arguments) : item.tooltip) + '"' : '') + '>' + ( item.getText ? item.getText.apply(item.scope||me.scope||me, arguments) : (item.text || me.text) ) + '</a>';
            }
